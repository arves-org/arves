...
function enc(out, v) {
  ...
  if (typeof v === 'bigint') { if (v >= 0n) head(out, 0, v); else head(out, 1, -1n - v); return; }
  ...
  if (Array.isArray(v)) { head(out, 4, BigInt(v.length)); for (const it of v) enc(out, it); return; }
  if (typeof v === 'object') { encMap(out, v); return; }
  throw new Error('ARVES: unsupported value kind');
}

/** Canonical dCBOR body of an ARVES value (ACS-002/1). */
export function encode(v) { const out = []; enc(out, v); return Uint8Array.from(out); }
