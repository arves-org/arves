...
> **RCR-040** (MODEL ↔ CODE conformance witness — closes the "model-checks the
> DESIGN, not the Rust code" gap that capped Verification. Implements the
> AKF-CS-1-style bridge from `ARVES_Kernel_Formal_Spec.md` §7: a Rust test that
> drives the REAL `RefKernel` and the REAL `ClusterKernel`/`ClusterSim` through the
> exact concrete scenarios the TLA+ models check, asserting the code's observable
> state matches each model invariant at each step — `ARVES_Kernel.tla` (OWN_001,
> ORCH_003_ReplayEquiv byte-exact `truth_hash 0x7bb9b2e30ee7427c`, ORCH_004,
> order-sensitivity) and `ARVES_Cluster.tla`'s five safety invariants
> (ElectionSafety, LogMatching, StateMachineSafety, LeaderCompleteness,
> LinearizableCommit) witnessed on a trace with a term-changing failover. Ships
> three read-only `ClusterSim` introspection accessors (`log_terms_of`,
> `leaders_by_term_of`, `committed_terms_of` — consensus mechanism metadata only,
> never Kernel truth, mutate nothing) + `tests/model_code_conformance.rs` (+3
> deterministic tests) + living evidence `verification/formal/MODEL_CODE_CONFORMANCE.md`
> (the invariant→assertion mapping). HONEST SCOPE: a concrete-SCENARIO conformance
> witness linking the checked model to the running code — NOT a full
> refinement/formal-code proof (TLC still owns all-interleavings; full
> model-to-code refinement via Kani/TLA+ trace-validation stays recorded OQ-1). No
> new distributed guarantee, no GA/multi-host claim. ZERO new third-party crates
> (reuses `arves-acs` SHA-256), no new dependency edge (LAYER-001 untouched), no
> frozen type/trait touched. Workspace **300/0** (+3). Record:
> `runtime/rcr/RCR-040.md`),
> **RCR-039** (GROUP-COMMIT / BATCHED-FSYNC WAL — closes the concrete deferred v1.1
> debt the L4 report MEASURED: the fsync-per-commit throughput ceiling. `arves-persistence`
> gains `Wal::append_group` (added to the trait WITH a default = per-record fallback;
> `FileWal` OVERRIDES it to write every frame then issue exactly ONE `sync_all` for the
> whole group — a group crossing a segment boundary syncs the outgoing segment before
> sealing it, so no frame is left un-synced; the default per-record `append` path is
> byte-identical/untouched). `arves-kernel` gains `RefKernel::commit_group` — the SAME
> gateway admission as `commit`/`commit_batch` (ORCH-004 idempotency, RCR-005
> content-integrity, IDR-004 cross-shard refusal, `BatchOutcome`/`BatchError` reused),
> validate-first → partition FRESH vs already-committed (intra-group duplicates collapse
> to one append) → ONE coalesced `append_group` → **ACK only AFTER the coalesced fsync**
> (no ack-before-durable; a crash mid-group leaves an un-acked tail — recovery truncates
> only the torn final frame while any fully-written un-acked frames survive and are
> idempotently reconciled on retry (ORCH-004), never acked-but-lost). DETERMINISM preserved (HARD RULE 4): committed content
> AND order equal N sequential commits, so grouped `truth_hash` == the non-grouped path
> (proven by unit test + the L4 gate; `FileWal` group segment bytes == N sequential
> appends). MEASURED (this host, single-node, single shard — `windows`/`x86_64`, group
> 64): commit throughput **~1,300 c/s (per-commit) → 33,700–51,000 c/s (group), 25.8–38.3×**,
> `truth_hash` byte-identical across both paths (and to the prior per-commit-only report);
> `verification/industrial/L4_REPORT.md` regenerated by re-running the L4 perf harness.
> fsync-count proved 1-per-group-not-N via a counting `WalStore` double. Additive, no new
> dependency, no new cross-crate edge (kernel(40)→persistence(20) pre-existing, LAYER-001
> downward-only); the frozen `Kernel` trait untouched; this lane adds **+13** group-commit
> tests (workspace **300/0** with RCR-040 concurrently integrated). Record:
> `runtime/rcr/RCR-039.md`).
