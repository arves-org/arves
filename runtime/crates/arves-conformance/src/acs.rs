...
fn negative_corpus() -> Vec<(&'static str, &'static str, Vec<u8>, &'static str)> {
    vec![
        ...
        // depth edge: 129 nested arrays puts the leaf at depth 129 — exactly one past
        // MAX_DEPTH=128 (ACS-002 §5.10), so it pins the reject boundary, not just "deep".
        ("nesting-too-deep", "core", { let mut v = vec![0x81u8; 129]; v.push(0x00); v }, "nesting-too-deep"),
