...
- **Machine-verifiable rows (Section A)** are regenerated by `evidence_probe.py`, which
  actually runs each suite — they cannot drift from reality. Last probe: **7/7 PASS**.
...
| **Robustness (whole-system destroy → repair → prove)** | Security, Reliability, Correctness | L2 | G1 | ✅ PASS | 21 confirmed blocker/major found & fixed across 6 files; 40 product regressions + Rust gate test lock them | `node products/robustness.test.mjs` · `cargo test --workspace` |
...
| **P6.5 Ecosystem SDK & Authoring Kit** (third-party enablement) | Ecosystem, Certification, Integrity | L2 | G1 | ✅ PASS | a third-party capability authored (Author SDK) → certified (conformance) → signed over the ACTUAL code (content-addressed) → installed (cert + code-integrity gated) → invoked → truth in the frozen Kernel; **runtime unchanged, no RCR** (metric #3) | `node products/arves-ecosystem-sdk/examples/third-party-capability.mjs` |
| **Cold Ecosystem Challenge** (independent author, Kit-only) | Ecosystem, **Independent** | L3 | **G1** | ✅ PASS | a fresh-context developer, forbidden from all internals, authored + certified a NEW capability (`support.triage`) from the Authoring Kit ALONE, first try; 8 Kit gaps surfaced → fixed | `node products/arves-ecosystem-sdk/bin/arves.mjs certify examples/ticket-triage.capability.mjs` |
| **P7 Marketplace** (distribution) | Ecosystem, Integrity | L2 | G1 | ✅ PASS | publish (cert-gated + signature-verified) → a DIFFERENT consumer installs + invokes on the frozen runtime; refuses uncertified/tampered/duplicate | `node products/arves-marketplace/examples/publish-install.mjs` |
| **Runtime certification / Independent Runtime Alliance** (P8) | Certification, **Independent**, Survivability | L3 | G1 | ✅ PASS | a maintainer-independent harness certifies **2 runtimes (Rust + Python) under ONE conformance from `standard/` alone** — a **G1 rehearsal** of the outlives-its-makers mechanism (G2 third-party = **NOT YET MET**, see Section C) | `python verification/certification/certify_runtime.py` |
| Rust workspace tests (I1 runtime + gates + ACS + bridge) | Behaviour, Formal | L2 | G0 | ✅ PASS | 65 tests | `cargo test --manifest-path runtime/Cargo.toml --workspace` |
