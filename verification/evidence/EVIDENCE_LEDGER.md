...
| **Robustness (whole-system destroy → repair → prove)** | Security, Reliability, Correctness | L2 | G1 | ✅ PASS | 21 confirmed blocker/major found & fixed across 6 files; 37 product regressions + Rust gate test lock them | `node products/robustness.test.mjs` · `cargo test --workspace` |
...
| **Runtime certification / Independent Runtime Alliance** (P8) | Certification, **Independent**, Survivability | L3 | G1 | ✅ PASS | a maintainer-independent harness certifies **2 runtimes (Rust + Python) under ONE conformance from `standard/` alone** — the "outlives its makers" proof | `python verification/certification/certify_runtime.py` |
